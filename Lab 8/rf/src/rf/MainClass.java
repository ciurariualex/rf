	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < evaluationSet.length; i++) {
				listToBeCalculatedClass
						.add(new Form(Double.valueOf(evaluationSet[i][0]), Double.valueOf(evaluationSet[i][1]),
								Double.valueOf(evaluationSet[i][2]), Double.valueOf(evaluationSet[i][3]), ""));
			}

			ArrayList<Form> list = new ArrayList<Form>();
			for (int i = 0; i < trainingSet.length; i++) {
				list.add(new Form(Double.valueOf(trainingSet[i][0]), Double.valueOf(trainingSet[i][1]),
						Double.valueOf(trainingSet[i][2]), Double.valueOf(trainingSet[i][3]), trainingSet[i][4]));
			}

			int[] k = new int[] { 1, 3, 5, 7, 9, 11, 13, 15 };
			...
			for (int u = 0; u < k.length; u++) {
				...
				for (int i = 0; i < listToBeCalculatedClass.size(); i++) {
					arrayOfDistancesList
							.add(DistanceUtils.calculateArrayOfDistance(listToBeCalculatedClass.get(i), list));
					Collections.sort(arrayOfDistancesList.get(i));
					listToBeCalculatedClass.get(i).Class = DistanceUtils.calculateClass(arrayOfDistancesList.get(i), k[u]);
				}

				int correctAnswer = 0;

				for (int i = 0; i < listToBeCalculatedClass.size(); i++) {
					String x = listToBeCalculatedClass.get(i).Class;
					String y = evaluationSet[i][4];
					if (x.equals(y)) {
						correctAnswer++;
					}
				}

				System.out.println("Pentru k = " + k[u] + " au fost gasite " + correctAnswer + " raspunsuri corecte din "
						+ listToBeCalculatedClass.size());
			}
